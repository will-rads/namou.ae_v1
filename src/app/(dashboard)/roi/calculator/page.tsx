        ...
        <div className="space-y-4">
          <ContentCard className="bg-forest text-white border-forest">
            ...
            <div className="space-y-4">
              ...
              <OutputMetric label="GFA Price" value={`AED ${outputs.gfaPrice.toFixed(2)}/sq ft`} />
            </div>
          </ContentCard>
    ...
    <div>
      <div className="flex items-center justify-between mb-1.5">
        ...
        <span className="text-sm font-semibold text-forest">
          {unit === "AED" ? `AED ${formatNumber(value)}` : `${value}${unit}`}
        </span>
      </div>
      <input
        type="range"
...
function OutputMetric({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div>
      ...
      <p className={`text-lg font-bold font-heading ${highlight ? "text-mint" : ""}`}>{value}</p>
    </div>
  );
}
