...
export default function ThankYouPage() {
  ...
  useEffect(() => {
    try {
      ...
      const image = sessionStorage.getItem("Assignee_image");
      if (name) setSpecialist(name);
      if (email) setSpecialistEmail(email);
      if (image) setSpecialistImage(image);
    } catch {}
  }, []);
  ...
  return (
    <div className="relative flex flex-col min-h-screen overflow-x-hidden">
      ...
      <div className="relative z-10 px-4 sm:px-8 pt-3 sm:pt-4">
        <Link href="/home">
          <Image
            src="/logo.png"
            ...
            width={320}
            height={90}
            className="object-contain w-auto h-[8vh] sm:h-[12vh]"
            priority
          />
        </Link>
      </div>
      ...
      {/* Content */}
      <div className="relative z-10 flex-1 flex flex-col items-center justify-start px-4 sm:px-8 pt-2 pb-4">
