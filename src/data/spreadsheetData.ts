...
// Source: https://docs.google.com/spreadsheets/d/1WVmxXfRnJijv1R1iHTdpPKcwpG-vUC3z/edit?gid=2114072871
...
export const SPREADSHEET_COLUMNS: SpreadsheetColumn[] = [
  ...
  { key: "galleryVideo1", label: "Gallery Video 1", width: "min-w-[250px]" },
  { key: "galleryVideo2", label: "Gallery Video 2", width: "min-w-[250px]" },
  // ── Site-essential fields (used by the frontend Plot model) ──
  { key: "category", label: "Category", width: "min-w-[140px]" },
  { key: "availability", label: "Availability", width: "min-w-[100px]" },
];
...
export function spreadsheetRowsToPlots(rows: SpreadsheetRow[]): Plot[] {
  ...
  return rows
    ...
    .map((row, index) => {
      ...
      // Category: normalize to lowercase slug, fallback to derivation from landUse
      const catRaw = (row.category || "").trim().toLowerCase();
      const category: LandCategory = catRaw || deriveCategoryFromLandUse(row.landUse || "");
      ...
      return {
        ...
        pricePerSqFt: parseNum(row.plotArea) > 0 ? Math.round(parseNum(row.askingPrice) / parseNum(row.plotArea)) : 0,
        landUse: row.landUse || "",
        ...
        ...(row.galleryVideo1 ? { galleryVideo1: row.galleryVideo1 } : {}),
        ...(row.galleryVideo2 ? { galleryVideo2: row.galleryVideo2 } : {}),
        ...(row.landRegFee ? { landRegFee: row.landRegFee } : {}),
        ...(row.landRegFeePct ? { landRegFeePct: row.landRegFeePct } : {}),
        ...(row.commissionFee ? { commissionFee: row.commissionFee } : {}),
        ...(row.commissionFeePct ? { commissionFeePct: row.commissionFeePct } : {}),
        ...(row.adminFee ? { adminFee: row.adminFee } : {}),
        ...(row.adminFeePct ? { adminFeePct: row.adminFeePct } : {}),
        ...(row.annualServiceCharge ? { annualServiceCharge: row.annualServiceCharge } : {}),
        available: (row.availability || "").trim().toUpperCase() !== "NO",
      } satisfies Plot;
    });
}
